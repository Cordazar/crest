...
function handleGet(req, res, next) {
  ...
  for (v in req.query) {
    if (test.indexOf(v) !== -1) {
      options[v] = isNaN(parseInt(req.query[v])) ? req.query[v] : parseInt(req.query[v]);
    }
  }
...
server.post('/:db/:collection', function (req, res) {
  ...
  if (req.params) {
    MongoClient.connect(util.connectionURL(req.params.db, config), function (err, db) {
      ...
      collection.insert(Array.isArray(req.params) ? util.cleanParams(req.params[0]) : util.cleanParams(req.params), function (err, result) {
        res.header('Location', '/' + req.params.db + '/' + req.params.collection + '/' + result.ops[0]._id.toHexString());
